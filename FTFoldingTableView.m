...
-(UIImage *)setBackgroungimage:(NSInteger )section
{
    if (_foldingDelegate && [_foldingDelegate respondsToSelector:@selector(ftFoldingTableView:setbackgroundImage:)]) {
        return [_foldingDelegate ftFoldingTableView:self setbackgroundImage:section];
    }
    return 0;
}
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
   ...
   [sectionHeaderView setupWithBackgroundColor:[self backgroundColorForSection:section] titleString:[self titleForSection:section] titleColor:[self titleColorForSection:section] titleFont:[self titleFontForSection:section] backgroungImage:[self setBackgroungimage:section] descriptionString:[self descriptionForSection:section] descriptionColor:[self descriptionColorForSection:section] descriptionFont:[self descriptionFontForSection:section] arrowImage:[self arrowImageForSection:section] arrowPosition:[self perferedArrowPosition] sectionState:((NSNumber *)self.statusArray[section]).integerValue];
...
-(void)setupSubviewsWithArrowPosition:(FTFoldingSectionHeaderArrowPosition)arrowPosition
{
    ...
    CGRect titleRect = CGRectMake(0, 0, labelWidth, labelHeight);
